...
public class InitiatePaymentRequestHandler {
	...
	private Map<String, String> prepareRequestFormPayload(StripeProviderRequest stripeProviderRequest) {
		...
		initiatePaymentRequestPayload.put("line_items[0][price_data][currency]", stripeProviderRequest.getCurrency());
		...
		initiatePaymentRequestPayload.put("line_items[0][price_data][unit_amount]",
				String.valueOf(stripeProviderRequest.getAmount() * 100).split("\\.")[0]);
		
		initiatePaymentRequestPayload.put("line_items[0][quantity]", String.valueOf(stripeProviderRequest.getQuantity()));
